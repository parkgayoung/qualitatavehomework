...
var s_account="wileyonlinelibrary-production"
...
s.linkInternalFilters="javascript:,onlinelibrary.wiley.com,olabout.wiley.com,ordering.onlinelibrary.wiley.com,doi.wiley.com,wiley.qualtrics.com,surveygizmo.com,eal.ol.wiley.com,wileyonlinelibrary.com,onlinelibrary-wiley-com,dev.wiley.com,qa.wiley.com,cochranelibrary.com,fepqua.wiley.com,wol2.wiley.com"
s.linkLeaveQueryString=false
s.linkTrackVars=""
s.linkTrackEvents="None"
s.usePlugins=true;

s.prop30=s.eVar37="D=g"; //dynamically assign URL
s.eVar31="D=User-Agent";


function s_doPlugins(s) {
		if(document.title) s.pageName=document.title.replace(" - Wiley Online Library","");
		s.events=s.apl(s.events,'event4',',',1); //pageview event
		...
		s.eVar6=s.getVisitNum();
		s.visEvent=s.getVisitStart("s_visit");
		if (s.visEvent) s.events=s.apl(s.events,'event2',',',1);

		// External Campaigns
        s.eVar13=(s.getQueryParam('cid').length > 0) ? s.getQueryParam('cid') : getHashQueryParam('cid');
		s.eVar14=(s.getQueryParam('cm').length > 0) ? s.getQueryParam('cm') : getHashQueryParam('cm');
		s.eVar15=(s.getQueryParam('cs').length > 0) ? s.getQueryParam('cs') : getHashQueryParam('cs');
		s.eVar16=(s.getQueryParam('cu').length > 0) ? s.getQueryParam('cu') : getHashQueryParam('cu');
		s.eVar17=(s.getQueryParam('cd').length > 0) ? s.getQueryParam('cd') : getHashQueryParam('cd');
		if(s.eVar13){
			if(s.eVar14) s.campaign='D=v13+"|"+v14+"|"+v15+"|"+v16+"|"+v17';
			else s.campaign = 'D=v13';
		}
		if(s.prop2 && s.prop2=="SPECIAL FEATURES"){ 
			s.filename = document.location.pathname.replace(/\?.*$/,"").replace(/.*\//,""); 
			s.prop2=s.filename; 
		}
                s.prop47 = document.location.hash;
		s.prop5 = s.prop2 ? 'D=c1+"|"+c2' : 'D=c1'; // dynamically concat prop1|prop2
		
		if(s.prop20){
			//search results - cleaned and delimited
			s.prop20 = s.prop20.toLowerCase().trim();
			s.prop22 = s.prop20.split(" ").join("|");
		}
		
		if( s.find(s.events,"event11",",",1) || s.find(s.events,"event12",",",1) || s.find(s.events,"event60",",",1)  || s.find(s.events,"event64",",",1) ){
			
			s.events=s.apl(s.events,'event62',',',1); // combined fulltext views for 3rd parties
		}

		s.prop71=s.getPreviousValue(s.channel,'s_prevChannel','');
		s.prop72=s.getPreviousValue(s.prop1,'s_prevProp1','');
		s.prop73=s.getPreviousValue(s.prop2,'s_prevProp2','');
...
/* Plugin: find in list */
s.find=new Function("L","v","d","u","var s=this,m=0;if(!L)L='';if(u){var i,n,a=s.split(L,d);for(i=0;i<a.length;i++){n=a[i];m = m || ( u==1? (n==v) : (n.toLowerCase()==v.toLowerCase()) );}} return m;");
...
/* s.customEvent - listen for custom event */
s.customEvent=Function("e","h","if(document.addEventListe"
+"ner){document.addEventListener(e,h,false);}else if(document.attachEv"
+"ent){document.documentElement.attachEvent('onpropertychange',functio"
+"n(evt){if(evt.propertyName==e){h();}});}");

s.customEvent("readcube.webviewer.open",function(){
s.events="";
s.linkTrackVars='';
s.prop1="FULLTEXT";
s.prop2="READCUBE FULLTEXT PDF";
s.contextData.eventName ="readcube.webviewer.open";
s.t();
s.contextData.eventName="";
});



s.trackEvent = function(eventName,eventData){
    s.linkTrackVars='';
    s.linkTrackEvents='';
	s.contextData.eventData=eventData;
	s.contextData.eventName=eventName;
	s.tl();
    s.contextData.eventData="";
    s.contextData.eventName="";
...
s.visitorNamespace = "wileypublishing"
s.trackingServer = "wileypublishing.112.2o7.net";
s.trackingServerSecure = "";



s.setTagContainer("OnlineLibrary_s");
